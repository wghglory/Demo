...
namespace Ajax.CRUD.DAL
{
    public class Aspx_StudentsDal
    {
        ...
        public List<Aspx_Students> GetStudentsBetweentAnd(int start, int end)
        {
            List<Aspx_Students> list = new List<Aspx_Students>();
            string sql = "select * from 	(select *,rn=row_number() over(order by stuId) from Aspx_Students) as t 	where t.rn between @sn and @en";
            SqlParameter[] pms = new SqlParameter[] { 
            new SqlParameter("@sn",start),
            new SqlParameter("@en",end)
            };
