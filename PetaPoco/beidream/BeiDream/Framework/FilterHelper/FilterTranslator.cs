...
namespace BeiDream.Framework.Common
{
    ...
    public class FilterTranslator
    {
        ...
        /// 需更新的字段列表
        /// </summary>
        private IList<FilterParam> updateParms;

        public IList<FilterParam> UpdateParms
        {
            get { return updateParms; }
            set { updateParms = value; }
        }
        /// <summary>
        /// 查询语句可变参数数组
        /// </summary>
        ...
        {
            this.CommandText = TranslateUpdate(this.UpdateParms);
        }
        /// <summary>
        /// 对多组规则进行翻译解析
        /// </summary>
        ...
        /// <returns></returns>
        public string TranslateUpdate(IList<FilterParam> list)
        {
            ...
            if (list != null)
            {
                foreach (FilterParam rule in list)
                {
                    if (appended)
                        bulider.Append(",");
        ...
        public string TranslateRule(FilterRule rule)
        {
            ...
            var op = rule.op.ToString().ToLower();
        ...
        /// <param name="rule">规则</param>
        /// <returns></returns>
        public string TranslateRule(FilterParam rule)
        {
            ...
            ////如果字段名采用了 用户信息参数
            //if (currentParmMatch.ContainsKey(rule.Name))
            //{
            //    var field = currentParmMatch[rule.Name]();
            //    bulider.Append(paramPrefixToken + CreateFilterParam(field, "int"));
            //}
            //else //这里实现了数据库实体条件的拼接,[ID]=xxx的形式
            //{
            ...
            //    if (IsEntityToSql)
            //    {
            //        bulider.Append(" it." + rule.Name + " ");
            //    }
            //    else
            //    {
            bulider.Append(leftToken + rule.Name + rightToken);
            //    }
            //}
            //操作符
            bulider.Append("=");
            ...
            //else
            //{
            bulider.Append(paramPrefixToken + CreateFilterParamUp(rule.Value, null, rule.Name));

            //}
            return bulider.ToString();
        }
        private string CreateFilterParamUp(object value, string type, string paramName)
        {

            paramName = "p" + ++paramCounter + paramName;
            object val = value;
        ...
        /// <param name="op"></param>
        /// <returns></returns> 
        public static string GetOperatorQueryText(GroupOperatorQueryEnum op)
        {
            switch (op.ToString().ToLower())
            {
                case "add":
                    return " + ";
                case "bitwiseand":
        ...
        /// <param name="op"></param>
        /// <returns></returns> 
        public static string GetOperatorQueryText(RuleOperatorQueryEnum op)
        {
            switch (op.ToString().ToLower())
            {
                case "add":
    ...
    /// 查询条件之间的操作关系枚举
    /// </summary>
    public enum GroupOperatorQueryEnum
    { 
        /// <summary>
        /// 查询条件必须同时成立
        /// </summary>
    ...
    /// 查询条件的操作枚举
    /// </summary>
    public enum RuleOperatorQueryEnum
    { 
        /// <summary>
        /// 相似
        /// </summary>
